...
export class HomebaseListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.updateSubscription = this.appEventsService.subscribe('updateHomebaseEvent',
      () => this.getListOfHomebases());
    this.deleteSubscription = this.appEventsService.subscribe('providerDeletedEvent', () =>
      this.getListOfHomebases());
    this.updateListSubscription = this.appEventsService.subscribe('newHomebase',
      () => this.getListOfHomebases());
  ...
  getListOfHomebases = () => {
    this.isLoading = true;
    this.homebaseService.getAllHomebases().subscribe((data: any) => {
      const {
        homebases, pageMeta: {
          totalResults
        }
      ...
      this.appEventsService.broadcast({
        name: 'updateHeaderTitle', content:
          { badgeSize: this.totalItems, actionButton: 'Add Homebase' }
      });
  ...
  ngOnDestroy(): void {
    SubscriptionHelper.unsubscribeHelper([this.updateSubscription, this.deleteSubscription, this.updateListSubscription]);
  }
}
