...
export class DashboardComponent implements OnInit {
  ...
  callTripService(tripType = '') {
    return this.tripService.getTripData(this.filter1, tripType);
  }
  ...
  getEmbassyVisits() {
    this.tripService.getTripData(this.filter1, 'Embassy Visit').subscribe(res => {
  ...
  getTripsAnalysis() {
    ...
    this.tripService.getTripData(this.filter1, 'Regular Trip').subscribe(res => {
      const { data: { trips } } = res;
